...
import { removeCookie, setCookie } from '../util/auth'
...
export const Mutation = {
  ...
  async signup (parent: any, args: any, context: Context) {
    ...
    setCookie(context.response, user.id)
    return { user }
  },
  ...
  async deleteTrip (parent: any, args: any, context: Context) {
    ...
    if (!trip) throw new Error(`No trip with ID ${args.id}`)
    const user = await context.prisma.trip({ id: args.id }).user()
    if (user.id !== userId) throw new Error('Cannot delete other users\' trips')
